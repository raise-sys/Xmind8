window.mindCanvas = (() => {
    ...
    const shortcut = event => {
        ...
        if (!altArrow && !ctrlPriority && !typing && !['F2', 'Enter', 'Tab', 'Delete', 'ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight'].includes(event.key)) return;
        event.preventDefault();
        event.stopPropagation();
        const shortcutKey = altArrow ? `Alt${event.key}` : ctrlPriority ? `Ctrl${event.key}` : typing ? `Type:${event.key}` : event.key;
        dotnet.invokeMethodAsync('HandleShortcut', shortcutKey);
    };
    ...
    const showEditor = (svg, editor, x, y, width, height, fontSize, textAlign) => {
    ...
    const selectFolder = async () => {
        if (!window.showDirectoryPicker) throw new Error('このブラウザはフォルダ保存に対応していません。EdgeまたはChromeを使用してください');
        saveDirectory = await window.showDirectoryPicker({ mode: 'readwrite' });
        return saveDirectory.name;
    };
    const listFolderFiles = async () => {
        if (!saveDirectory) throw new Error('先に保存先フォルダを選択してください');
        const files = [];
        for await (const [name, handle] of saveDirectory.entries())
            if (handle.kind === 'file' && name.toLowerCase().endsWith('.json')) files.push(name);
        return files.sort((a, b) => a.localeCompare(b, 'ja'));
    };
    const saveToFolder = async (name, contents) => {
        if (!saveDirectory) throw new Error('先に保存先フォルダを選択してください');
        const safeName = name.replace(/[\\/:*?"<>|]/g, '_');
        const fileHandle = await saveDirectory.getFileHandle(safeName, { create: true });
        const writable = await fileHandle.createWritable();
        await writable.write(contents);
        await writable.close();
    };
    const loadFromFolder = async name => {
        if (!saveDirectory) throw new Error('先に保存先フォルダを選択してください');
        const fileHandle = await saveDirectory.getFileHandle(name);
    ...
    return {
        ...
        dispose: () => { window.removeEventListener('paste', paste, true); window.removeEventListener('keydown', shortcut, true); dotnet = null; },
        readClipboard,
        readYoutubeLink,
        startNodeDrag,
        startCanvasPan,
